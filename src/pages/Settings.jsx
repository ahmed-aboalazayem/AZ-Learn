            ...
            <div className="flex items-center gap-8 border-b border-outline-variant mb-8 overflow-x-auto no-scrollbar select-none">
                {["Profile", "Notifications", "Appearance", "Security"].map((tab) => (
                    <button
                        ...
                        className={`pb-4 font-label-md text-label-md cursor-pointer transition-all border-none bg-transparent ${activeTab === tab
                    ...
                    {activeTab === "Profile" && (
                        <form onSubmit={handleSaveProfile} className="bg-surface-container-lowest rounded-xl shadow-[0_4px_24px_rgba(0,0,0,0.04)] border border-surface-container-highest p-8 space-y-8">
                            ...
                            <div className="flex items-center gap-6">
                                ...
                                <div className="flex flex-col gap-3">
                                    <div className="flex gap-3">
                                        <button onClick={handleUploadAvatar} type="button" className="bg-surface-container-lowest border border-outline-variant text-primary px-4 py-2 rounded-lg font-label-md text-label-md hover:bg-surface-container-low transition-colors shadow-sm cursor-pointer font-semibold">
                                            Upload New
                                        </button>
                                        <button onClick={handleRemoveAvatar} type="button" className="px-4 py-2 rounded-lg font-label-md text-label-md text-error hover:bg-error-container transition-colors cursor-pointer border-none bg-transparent font-semibold">
                                            Remove
                                        </button>
                                    </div>
                            ...
                            <div className="flex justify-end pt-2">
                                <button type="submit" className="bg-gradient-to-r from-[#6C63FF] to-[#3B82F6] text-black px-6 py-2.5 rounded-lg font-label-md text-label-md shadow-md hover:shadow-lg transition-all border-none cursor-pointer font-semibold">
                                Save Changes
                            </button>
                        </div>
                        </form>
                    ...
                    <div className="bg-surface-container-lowest rounded-xl shadow-[0_4px_24px_rgba(0,0,0,0.04)] border border-surface-container-highest p-8 space-y-6">
                        ...
                        <div className="grid grid-cols-2 gap-4 mt-4">
                            <div
                                ...
                                className={`border rounded-xl p-4 flex flex-col items-center justify-center gap-3 cursor-pointer transition-all ${profile.theme === "light"
                                        ? "border-primary bg-primary-container/10 text-primary font-bold"
                                        : "border-outline-variant hover:bg-slate-50 text-on-surface-variant"
                                    }`}
                            >
                                <span className={`material-symbols-outlined text-3xl ${profile.theme === "light" ? "text-primary" : "text-slate-500"}`}>light_mode</span>
                                <span className="text-sm">Light Theme</span>
                            </div>
                            <div
                                onClick={() => updateProfile({ theme: "dark" })}
                                className={`border rounded-xl p-4 flex flex-col items-center justify-center gap-3 cursor-pointer transition-all ${profile.theme === "dark"
                                        ? "border-primary bg-primary-container/10 text-primary font-bold"
                                        : "border-outline-variant hover:bg-slate-50 text-on-surface-variant"
                                    }`}
                            >
                                <span className={`material-symbols-outlined text-3xl ${profile.theme === "dark" ? "text-primary" : "text-slate-500"}`}>dark_mode</span>
